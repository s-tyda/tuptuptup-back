...
def create_post(
    *, db: Session = Depends(get_db), post_in: schemas.PostCreate
) -> Any:
...
def update_post(
    *, db: Session = Depends(get_db), uuid: UUID4, post_in: schemas.PostUpdate
) -> Any:
    post = actions.post.get(db=db, uuid=uuid)
    if not post:
        raise HTTPException(
...
def get_post(*, db: Session = Depends(get_db), uuid: UUID4) -> Any:
    post = actions.post.get(db=db, uuid=uuid)
    if not post:
        raise HTTPException(
            status_code=HTTP_404_NOT_FOUND, detail="Post not found"
...
def delete_post(*, db: Session = Depends(get_db), uuid: UUID4) -> Any:
    post = actions.post.get(db=db, uuid=uuid)
    if not post:
        raise HTTPException(
            status_code=HTTP_404_NOT_FOUND, detail="Post not found"
        )
    post = actions.post.remove(db=db, uuid=uuid)
    return post
